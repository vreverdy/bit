...
class bit_reference
{
    ...
    public:
    template <class T> bit_reference& operator+=(const T& val) noexcept;
    template <class T> bit_reference& operator-=(const T& val) noexcept;
    template <class T> bit_reference& operator*=(const T& val) noexcept;
    template <class T> bit_reference& operator/=(const T& val) noexcept;
    template <class T> bit_reference& operator%=(const T& val) noexcept;
...
// -------------- BIT REFERENCE: COMPOUND ASSIGNMENT OPERATORS -------------- //
// Adds and assigns
template <class UIntType>
template <class T> 
bit_reference<UIntType>& bit_reference<UIntType>::operator+=(
...
) noexcept
{
    ...
    if (std::is_arithmetic<T>::value && std::is_integral<type>::value) {
        if (static_cast<bit_value::byte_type>(val) & 1) {
            flip();
        }
    } else {
        *this = *this + val;
...
) noexcept
{
    using type = decltype(std::declval<bit_value>() * std::declval<T>());
    if (std::is_arithmetic<T>::value && std::is_integral<type>::value) {
        if (*this && !(static_cast<bit_value::byte_type>(val) & 1)) {
            reset();
        }
    } else {
        *this = *this * val;
...
// ------------ BIT REFERENCE: INCREMENT AND DECREMENT OPERATORS ------------ //
// Increments the bit reference and returns it
template <class UIntType>
bit_reference<UIntType>& bit_reference<UIntType>::operator++(
) noexcept
{
...
// Decrements the bit reference and returns it
template <class UIntType>
bit_reference<UIntType>& bit_reference<UIntType>::operator--(
) noexcept
{
...
// Increments the bit reference and returns the old value
template <class UIntType>
bit_value bit_reference<UIntType>::operator++(
    int
) noexcept
{
    bit_value old = *this;
...
// Decrements the bit reference and returns the old value
template <class UIntType>
bit_value bit_reference<UIntType>::operator--(
    int
) noexcept
